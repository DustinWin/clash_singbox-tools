...
# 一、 说明
每天早上 3 点（北京时间）自动构建生成 Clash.Meta Alpha 版内核（arm64 和 amd64）、Clash.Meta Release 版内核（arm64 和 amd64）和 AdGuardHome（arm64）
# 二、 使用方法
...
mkdir -p $clashdir/ui && chmod +x $clashdir/ui && rm -rf $clashdir/ui/*
tar -xzvf ./tmp/Yacd-meta.tar.gz -C $clashdir/ui && rm -f ./tmp/Yacd-meta.tar.gz
...
30 3 * * 1,3,5 curl -o ./tmp/Yacd-meta.tar.gz -L https://github.com/DustinWin/clash-tools/raw/main/Yacd-meta/Yacd-meta.tar.gz && mkdir -p /data/clash/ui && chmod +x /data/clash/ui && rm -rf /data/clash/ui/* && tar -xzvf ./tmp/Yacd-meta.tar.gz -C $clashdir/ui && rm -f ./tmp/Yacd-meta.tar.gz && /data/clash/start.sh restart >/dev/null 2>&1 #每周一、三、五早上 3 点半更新 Yacd-meta 面板
